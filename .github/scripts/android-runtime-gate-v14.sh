...
collect_diag() {
  ...
  adb logcat -d -b all -t 8000 > evidence/runtime/diag-logcat.txt 2>&1 || true
...
wait_framework() {
  ...
  for i in $(seq 1 120); do
    ...
    if [ -n "$s" ] && [ -n "$f" ] && [ "$s" = "$prev_s" ] && [ "$f" = "$prev_f" ] && echo "$a" | grep -q found && echo "$p" | grep -q found; then
      stable=$((stable+1))
    else
      stable=0
    fi
...
query_registration() {
  local app_path inst
  app_path=$(adb shell pm path "$PKG" 2>/dev/null | tr -d '\r' || true)
  inst=$(adb shell pm list instrumentation 2>/dev/null | sed -n 's/^instrumentation:\([^ ]*\).*target=com.veltrix.hom.vnext.dev.*/\1/p' | head -1 | tr -d '\r' || true)
  printf '%s\n%s\n' "$app_path" "$inst"
}

ensure_runtime_registration() {
  local label="$1" poll app_path inst need_app=0 need_test=0
  wait_framework "${label}-framework"
  : > "evidence/runtime/registration-${label}.txt"

  # A userspace/framework restart can bring PackageManager back before /data apps
  # are rescanned. Give it a real rescan window before deciding recovery is needed.
  for poll in $(seq 1 30); do
    app_path=$(adb shell pm path "$PKG" 2>/dev/null | tr -d '\r' || true)
    inst=$(adb shell pm list instrumentation 2>/dev/null | sed -n 's/^instrumentation:\([^ ]*\).*target=com.veltrix.hom.vnext.dev.*/\1/p' | head -1 | tr -d '\r' || true)
    printf 'poll=%s app=%s instrumentation=%s\n' "$poll" "$app_path" "$inst" | tee -a "evidence/runtime/registration-${label}.txt"
    if echo "$app_path" | grep -q '^package:' && [ -n "$inst" ]; then
      I="$inst"
      echo "REGISTRATION_PRESENT label=$label" | tee -a "evidence/runtime/registration-${label}.txt"
      return 0
    fi
    sleep 2
  done
  ...
  app_path=$(adb shell pm path "$PKG" 2>/dev/null | tr -d '\r' || true)
  inst=$(adb shell pm list instrumentation 2>/dev/null | sed -n 's/^instrumentation:\([^ ]*\).*target=com.veltrix.hom.vnext.dev.*/\1/p' | head -1 | tr -d '\r' || true)
  echo "$app_path" | grep -q '^package:' || need_app=1
  [ -n "$inst" ] || need_test=1
  printf 'RECOVERY_REQUIRED label=%s app=%s test=%s\n' "$label" "$need_app" "$need_test" | tee -a "evidence/runtime/registration-${label}.txt"

  if [ "$need_app" -eq 1 ]; then install_apk "recovery-${label}-app" "$APK"; fi
  if [ "$need_test" -eq 1 ] || [ "$need_app" -eq 1 ]; then install_apk "recovery-${label}-test" "$TEST_APK"; fi
  wait_framework "${label}-post-reinstall"

  for poll in $(seq 1 20); do
    app_path=$(adb shell pm path "$PKG" 2>/dev/null | tr -d '\r' || true)
    inst=$(adb shell pm list instrumentation 2>/dev/null | sed -n 's/^instrumentation:\([^ ]*\).*target=com.veltrix.hom.vnext.dev.*/\1/p' | head -1 | tr -d '\r' || true)
    printf 'recovery_poll=%s app=%s instrumentation=%s\n' "$poll" "$app_path" "$inst" | tee -a "evidence/runtime/registration-${label}.txt"
    if echo "$app_path" | grep -q '^package:' && [ -n "$inst" ]; then
      I="$inst"
      echo "REGISTRATION_RECOVERED label=$label" | tee -a "evidence/runtime/registration-${label}.txt"
      return 0
    fi
    sleep 2
  done
  echo "REGISTRATION_RECOVERY_FAILED label=$label" | tee -a "evidence/runtime/registration-${label}.txt"
  return 1
}

launch_verified() {
  local label="$1" attempt sb fb sa fa pid state out restarted
  : > "evidence/runtime/launch-${label}.txt"
  for attempt in $(seq 1 10); do
    ensure_runtime_registration "${label}-attempt-${attempt}"
    sb=$(adb shell pidof system_server 2>/dev/null | tr -d '\r' || true)
    fb=$(adb shell pidof surfaceflinger 2>/dev/null | tr -d '\r' || true)
    adb logcat -c || true
    adb shell am force-stop "$PKG" || true
    set +e
    out=$(timeout 20s adb shell am start -W -n "$CMP" 2>&1)
    rc=$?
    set -e
    printf 'attempt=%s before_system=%s before_surface=%s rc=%s\n%s\n' "$attempt" "$sb" "$fb" "$rc" "$out" | tee -a "evidence/runtime/launch-${label}.txt"
    restarted=0
    ...
    sa=$(adb shell pidof system_server 2>/dev/null | tr -d '\r' || true)
    fa=$(adb shell pidof surfaceflinger 2>/dev/null | tr -d '\r' || true)
    adb logcat -d -b all -t 3000 > "evidence/runtime/launch-${label}-${attempt}-logcat.txt" 2>&1 || true
    if [ "$restarted" -eq 1 ] || [ "$sa" != "$sb" ] || [ "$fa" != "$fb" ] || echo "$out" | grep -Eqi 'broken pipe|dead object|device offline|closed|transport'; then
      echo "TRANSIENT_LAUNCH_RETRY attempt=$attempt" | tee -a "evidence/runtime/launch-${label}.txt"
      continue
    fi
    # Stable framework + registered package + failed Activity is a real app/runtime failure.
    ...
    echo "PACKAGE_DISAPPEARED_WITHOUT_OBSERVED_RESTART attempt=$attempt" | tee -a "evidence/runtime/launch-${label}.txt"
    return 1
  done
  return 1
}
...
run_test() {
  ...
  for attempt in $(seq 1 8); do
    ...
    set +e
    if [ -n "$classarg" ]; then
      out=$(timeout "${max_seconds}s" adb shell am instrument -w -e class "$classarg" "$I" 2>&1)
    else
      out=$(timeout "${max_seconds}s" adb shell am instrument -w "$I" 2>&1)
    fi
    rc=$?
    set -e
    ...
    sa=$(adb shell pidof system_server 2>/dev/null | tr -d '\r' || true)
    fa=$(adb shell pidof surfaceflinger 2>/dev/null | tr -d '\r' || true)
    if [ "$rc" -eq 0 ] && echo "$out" | grep -Fq "$marker"; then
      cp "evidence/runtime/${label}-attempt-${attempt}.txt" "evidence/runtime/${label}.txt"
      return 0
    fi
    if [ "$sa" != "$sb" ] || [ "$fa" != "$fb" ] || echo "$out" | grep -Eqi 'broken pipe|dead object|device offline|closed|transport'; then
...
AVD="veltrix_v14_api37_${GPU_MODE}"
echo no | avdmanager create avd -n "$AVD" -k 'system-images;android-37.0;google_apis;x86_64' -f
...
# Exact APK identity is rechecked inside the device job before any install/recovery.
...
wait_framework boot
echo "V14_API37_BOOT=PASS gpu=$GPU_MODE" | tee evidence/runtime/boot-gate.txt

install_apk app "$APK"
install_apk test "$TEST_APK"
ensure_runtime_registration initial
...
echo V14_INSTALL=PASS | tee evidence/runtime/install-gate.txt

launch_verified cold
echo V14_COLD_LAUNCH=PASS | tee evidence/runtime/cold-gate.txt
run_test shell 'OK (1 test)' 'com.veltrix.hom.vnext.ShellInstrumentedTest' 120
echo V14_SHELL=PASS | tee evidence/runtime/shell-gate.txt
...
echo V14_SERVER_DEVICE=PASS | tee evidence/runtime/server-device-gate.txt
...
echo V14_PROCESS_DEATH=PASS | tee evidence/runtime/process-gate.txt

ensure_runtime_registration before-offline
...
echo V14_OFFLINE=PASS | tee evidence/runtime/offline-gate.txt
...
ensure_runtime_registration after-network-restore
adb reverse tcp:8080 tcp:8080

full_ok=0
for attempt in $(seq 1 7); do
  ensure_runtime_registration "full-pre-${attempt}"
  ...
  sa=$(adb shell pidof system_server 2>/dev/null | tr -d '\r' || true)
  fa=$(adb shell pidof surfaceflinger 2>/dev/null | tr -d '\r' || true)
  if [ "$rc" -eq 0 ] && echo "$out" | grep -Fq 'OK (6 tests)'; then
    cp "evidence/runtime/full-connected-attempt-${attempt}.txt" evidence/runtime/connected.txt
    full_ok=1
    break
  fi
  if [ "$sa" != "$sb" ] || [ "$fa" != "$fb" ] || echo "$out" | grep -Eqi 'broken pipe|dead object|device offline|closed|transport'; then
    echo "TRANSIENT_FRAMEWORK_RETRY full attempt=$attempt system=$sb->$sa surface=$fb->$fa" | tee -a evidence/runtime/full-retries.txt
    continue
  fi
...
test "$full_ok" -eq 1
echo V14_CONNECTED_6_TESTS=PASS | tee evidence/runtime/connected-gate.txt
launch_verified final
echo V14_FINAL_LAUNCH=PASS | tee evidence/runtime/final-launch-gate.txt
echo "V14_ANDROID_RUNTIME_ALL=PASS gpu=$GPU_MODE" | tee evidence/runtime/runtime-all.txt
trap - EXIT
collect_diag
