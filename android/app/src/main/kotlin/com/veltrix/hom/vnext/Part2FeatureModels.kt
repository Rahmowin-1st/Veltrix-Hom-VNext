...
/** Frontend-only read models for Part 2. Backend remains the authority for every mutation. */
...
data class ProjectWorkspaceUiModel(
    val project: ProjectCardModel,
    val goals: List<ProjectGoalModel>,
    val recentChats: List<ConversationUiModel>,
    val sourceCount:Int,
    val noteCount:Int,
    ...
    val projectMemorySignals:Int,
    val meaningfulEvents:Long,
    val instruction:String?,
    val recommendationActions:List<String>,
    val contextTopic:String?,
    val contextLearningMode:String?,
    val revision:Long,
)
...
data class AssessmentDetailUiModel(
    val id:String,
    val kind:String,
    val title:String,
    val projectId:String?,
    ...
    val revision:Long,
    val questions:List<AssessmentQuestionUiModel>,
)
...
data class PracticeItemUiModel(
    val id:String,
    val position:Int,
    val prompt:String,
    val itemType:String,
...
data class PracticeSessionUiModel(
    val id:String,
    val projectId:String?,
    ...
    val items:List<PracticeItemUiModel>,
    val summary:String,
)
...
data class AvatarCatalogUiModel(
    val avatarId:String,
    val name:String,
    val assetKey:String,
    val tier:String,
...
data class PersonalMapUiModel(
    ...
    val units:List<MapUnitUiModel>,
    val revision:Long,
)
data class GameProfileUiModel(
    val level:Int,
    val lifetimeXp:Long,
...
class Part2FeatureRepository(
    ...
    suspend fun search(session:ApiSession,query:String,projectId:String?=null):RepositoryState<List<SearchUiModel>> = withContext(Dispatchers.IO) {
        ...
        try { val body=JSONObject().put("query",query.trim()).put("limit",50).apply{projectId?.let{put("projectId",it)}}.toString();val(_,text)=expect(api.request("POST","/v1/search",session.token,body));RepositoryState(parseSearch(JSONArray(text)),DataFreshness.FRESH) }
    ...
    suspend fun citations(session:ApiSession,conversationId:String,messageId:String):List<CitationUiModel> = withContext(Dispatchers.IO){val(_,text)=expect(api.request("GET","/v1/chats/$conversationId/messages/$messageId/citations",session.token,null));parseCitations(JSONArray(text))}

    suspend fun createTextSource(session:ApiSession,title:String,text:String):SourceUiModel = withContext(Dispatchers.IO){
        val bytes=text.toByteArray();val hash=java.security.MessageDigest.getInstance("SHA-256").digest(bytes).joinToString(""){"%02x".format(it.toInt() and 0xff)}
        ...
        val(_,created)=expect(api.request("POST","/v1/sources",session.token,meta.toString()),setOf(201));val id=JSONObject(created).getString("id");val(_,ready)=expect(api.request("POST","/v1/sources/$id/text",session.token,JSONObject().put("text",text).toString()));parseSource(JSONObject(ready))
    }
    ...
    suspend fun createPractice(session:ApiSession,projectId:String?,focusTopic:String?):PracticeSessionUiModel=withContext(Dispatchers.IO){val body=JSONObject().put("difficulty",2).put("targetItemCount",8).put("adaptive",true).apply{projectId?.let{put("projectId",it)};focusTopic?.takeIf{it.isNotBlank()}?.let{put("focusTopic",it)}};val(_,created)=expect(api.request("POST","/v1/practice",session.token,body.toString()),setOf(201));val id=JSONObject(created).getString("id");practice(session,id).value?:throw BackendUiException(500,"PRACTICE_PARSE",false,"Practice response unavailable")}
    ...
    suspend fun practiceHint(session:ApiSession,sessionId:String,itemId:String):String=withContext(Dispatchers.IO){val(_,t)=expect(api.request("POST","/v1/practice/$sessionId/items/$itemId/hint",session.token,"{}"));JSONObject(t).optString("body")}
    suspend fun practiceCheck(session:ApiSession,sessionId:String,itemId:String)=withContext(Dispatchers.IO){val(_,t)=expect(api.request("POST","/v1/practice/$sessionId/items/$itemId/check",session.token,"{}"));parsePracticeCheck(JSONObject(t))}
    ...
    suspend fun completePractice(session:ApiSession,sessionId:String,expectedRevision:Long):PracticeCompleteUiModel=withContext(Dispatchers.IO){val(_,t)=expect(api.request("POST","/v1/practice/$sessionId/complete",session.token,JSONObject().put("expectedRevision",expectedRevision).toString()));parsePracticeComplete(JSONObject(t))}

    suspend fun reviewFlashcard(session:ApiSession,cardId:String,rating:String):FlashcardUiModel=withContext(Dispatchers.IO){val(_,t)=expect(api.request("POST","/v1/flashcards/cards/$cardId/review",session.token,JSONObject().put("rating",rating).toString()));parseFlashcard(JSONObject(t))}
    ...
    suspend fun practiceFromMistake(session:ApiSession,mistakeId:String):String=withContext(Dispatchers.IO){val(_,t)=expect(api.request("POST","/v1/mistakes/$mistakeId/practice",session.token,JSONObject().put("idempotencyKey","android-mistake-practice-${UUID.randomUUID()}").toString()),setOf(201));JSONObject(t).getString("id")}
    ...
    fun streamAi(session:ApiSession,conversationId:String,projectId:String?,sourceIds:List<String>,text:String,learningMode:String,onEvent:(StreamUiEvent)->Unit){
        ...
        connection.outputStream.bufferedWriter().use{it.write(body)};val code=connection.responseCode
        if(code !in 200..299){val detail=connection.errorStream?.bufferedReader()?.use{it.readText()}.orEmpty();connection.disconnect();throw error(code,detail)}
        var eventType="message"
        connection.inputStream.bufferedReader().useLines{lines->lines.forEach{line->when{line.startsWith("event:")->eventType=line.substringAfter(':').trim();line.startsWith("data:")->runCatching{JSONObject(line.substringAfter(':').trim())}.getOrNull()?.let{obj->val err=obj.optJSONObject("error")?:if(eventType=="error")obj else null;onEvent(StreamUiEvent(eventType,obj.optNullable("messageId"),obj.optNullable("segment"),obj.optBoolean("final",false),err?.optString("code")?.takeIf{it.isNotBlank()},err?.optBoolean("retryable",false)==true))}}}}
    ...
    private suspend fun <T> cachedObject(session:ApiSession,kind:String,scope:String,path:String,force:Boolean,parser:(JSONObject)->T):RepositoryState<T> = withContext(Dispatchers.IO){val cached=snapshots.get(session.accountId,kind,scope);if(!force&&cached!=null&&System.currentTimeMillis()-cached.fetchedAtEpochMs<15_000)return@withContext RepositoryState(parser(JSONObject(cached.payload)),DataFreshness.FRESH,serverRevision=cached.serverRevision);try{val(_,text)=expect(api.request("GET",path,session.token,null));val o=JSONObject(text);val revision=o.optLong("revision",0);snapshots.put(Part3SnapshotEntity(session.accountId,kind,scope,text,revision,System.currentTimeMillis()));RepositoryState(parser(o),DataFreshness.FRESH,serverRevision=revision)}catch(e:BackendUiException){if(cached!=null)RepositoryState(parser(JSONObject(cached.payload)),DataFreshness.STALE,errorCode=e.code,retryable=e.retryable,serverRevision=cached.serverRevision)else RepositoryState(null,DataFreshness.STALE,errorCode=e.code,retryable=e.retryable)}catch(_:Throwable){if(cached!=null)RepositoryState(parser(JSONObject(cached.payload)),DataFreshness.OFFLINE,errorCode="OFFLINE",retryable=true,serverRevision=cached.serverRevision)else RepositoryState(null,DataFreshness.OFFLINE,errorCode="OFFLINE",retryable=true)}}
    private suspend fun <T> cachedArray(session:ApiSession,kind:String,scope:String,path:String,force:Boolean,parser:(JSONArray)->T):RepositoryState<T> = withContext(Dispatchers.IO){val cached=snapshots.get(session.accountId,kind,scope);if(!force&&cached!=null&&System.currentTimeMillis()-cached.fetchedAtEpochMs<15_000)return@withContext RepositoryState(parser(JSONArray(cached.payload)),DataFreshness.FRESH,serverRevision=cached.serverRevision);try{val(_,text)=expect(api.request("GET",path,session.token,null));val a=JSONArray(text);val revision=maxRevision(a);snapshots.put(Part3SnapshotEntity(session.accountId,kind,scope,text,revision,System.currentTimeMillis()));RepositoryState(parser(a),DataFreshness.FRESH,serverRevision=revision)}catch(e:BackendUiException){if(cached!=null)RepositoryState(parser(JSONArray(cached.payload)),DataFreshness.STALE,errorCode=e.code,retryable=e.retryable,serverRevision=cached.serverRevision)else RepositoryState(null,DataFreshness.STALE,errorCode=e.code,retryable=e.retryable)}catch(_:Throwable){if(cached!=null)RepositoryState(parser(JSONArray(cached.payload)),DataFreshness.OFFLINE,errorCode="OFFLINE",retryable=true,serverRevision=cached.serverRevision)else RepositoryState(null,DataFreshness.OFFLINE,errorCode="OFFLINE",retryable=true)}}
...
private fun parseProjects(a:JSONArray)=a.objects().map(::parseProject)
...
private fun parseWorkspace(o:JSONObject):ProjectWorkspaceUiModel{val instruction=o.optJSONObject("activeInstruction")?.optString("body")?.takeIf{it.isNotBlank()};val recommendations=o.optJSONArray("recommendations")?.objects()?.mapNotNull{it.optString("action").takeIf{v->v.isNotBlank()}}?:emptyList();val carry=o.optJSONObject("contextCarry");return ProjectWorkspaceUiModel(parseProject(o.getJSONObject("project")),o.optJSONArray("goals").orEmpty().objects().map(::parseGoal),o.optJSONArray("recentChats").orEmpty().objects().map(::parseConversation),o.optInt("sourceCount"),o.optInt("noteCount"),o.optInt("assessmentCount"),o.optInt("flashcardCount"),o.optInt("mistakeCount"),o.optInt("practiceCount"),o.optInt("projectMemorySignals"),o.optLong("meaningfulEvents"),instruction,recommendations,carry?.optNullable("topic"),carry?.optNullable("learningMode"),o.optLong("revision"))}
private fun parseChats(a:JSONArray)=a.objects().map(::parseConversation)
...
private fun parseMessages(a:JSONArray)=a.objects().map{ChatMessageUiModel(it.getString("id"),it.getString("conversationId"),it.optString("role"),it.optString("state"),it.optString("content"),it.optBoolean("finalMarker"),it.optLong("revision"),it.optString("createdAt"))}
private fun parseCitations(a:JSONArray)=a.objects().map{val c=it.optJSONObject("citation")?:JSONObject();CitationUiModel(it.optInt("index"),c.optString("sourceId"),c.optString("excerpt"),c.optInt("page").takeIf{c.has("page")&&!c.isNull("page")},c.optNullable("section"),c.optDouble("relevance",0.0))}
private fun parseSources(a:JSONArray)=a.objects().map(::parseSource)
...
private fun parseSearch(a:JSONArray)=a.objects().map{SearchUiModel(it.optString("type"),it.optString("id"),it.optString("title"),it.optString("snippet"),it.optNullable("projectId"),it.optString("deepLink"),it.optDouble("score"))}
private fun parseActivityResponse(o:JSONObject)=o.optJSONArray("items").orEmpty().objects().map{ActivityUiModel(it.optString("eventId"),it.optString("type"),it.optString("occurredAt"),it.optNullable("projectId"),it.optNullable("objectId"),it.optBoolean("meaningful"),it.optNullable("deepLink"))}
private fun parseAssessment(o:JSONObject):AssessmentDetailUiModel{val a=o.getJSONObject("assessment");return AssessmentDetailUiModel(a.getString("id"),a.optString("kind"),a.optString("title"),a.optNullable("projectId"),a.optString("state"),a.optInt("questionCount"),a.optLong("revision"),o.optJSONArray("questions").orEmpty().objects().map{AssessmentQuestionUiModel(it.getString("id"),it.optInt("position"),it.optString("prompt"),it.optString("type"),it.optJSONArray("options").stringValues())})}
...
private fun parseAssessmentResult(o:JSONObject)=AssessmentResultUiModel(parseAttempt(o.getJSONObject("attempt")),o.optDouble("score"),o.optDouble("accuracy"),o.optJSONArray("incorrectQuestionIds").stringValues(),o.optJSONArray("mistakeIds").stringValues())
private fun parsePractice(o:JSONObject):PracticeSessionUiModel{val s=o.getJSONObject("session");return PracticeSessionUiModel(s.getString("id"),s.optNullable("projectId"),s.optNullable("focusTopic"),s.optString("state"),s.optLong("revision"),o.optInt("currentPosition"),o.optInt("targetItemCount"),o.optInt("difficulty"),o.optBoolean("adaptive"),o.optString("hintPolicy"),o.optString("revealPolicy"),o.optJSONArray("items").orEmpty().objects().map(::parsePracticeItem),o.optString("summaryJson"))}
...
private fun parseFlashcards(a:JSONArray)=a.objects().map(::parseFlashcard)
...
private fun parseMistakes(a:JSONArray)=a.objects().map{MistakeUiModel(it.getString("id"),it.optNullable("projectId"),it.optNullable("sourceId"),it.optString("topic"),it.optString("prompt"),it.optNullable("userAnswer"),it.optNullable("expectedAnswer"),it.optInt("occurrenceCount"),it.optString("status"),it.optLong("revision"))}
private fun parseStore(o:JSONObject)=StoreCatalogUiModel(o.optString("catalogVersion"),o.optLong("coinBalance"),o.optJSONArray("items").orEmpty().objects().map{StoreItemUiModel(it.optString("itemId"),it.optString("itemType"),it.optLong("priceCoins"),it.optBoolean("owned"),it.optBoolean("available"),it.optString("requirements"),it.optString("metadata"))})
private fun parseInventory(a:JSONArray)=a.objects().map{InventoryItemUiModel(it.optString("itemId"),it.optString("type"),it.optString("ownershipSource"),it.optString("acquiredAt"),it.optLong("quantity"),it.optString("metadata"),it.optLong("revision"))}
private fun parseAvatars(a:JSONArray)=a.objects().map{AvatarCatalogUiModel(it.optString("avatarId"),it.optString("permanentName").ifBlank{it.optString("avatarId")},it.optString("assetKey"),it.optString("tier"),it.optBoolean("owned"),it.optBoolean("equipped"),if(it.has("storePrice")&&!it.isNull("storePrice"))it.optLong("storePrice") else null,it.optString("catalogVersion"),it.optString("identityMetadataJson"))}
private fun parseMap(o:JSONObject):PersonalMapUiModel{val e=o.optJSONObject("eligibility")?:JSONObject();return PersonalMapUiModel(o.optNullable("mapId"),o.optString("mapDefinitionId"),o.optInt("mapVersion"),o.optString("state"),e.optBoolean("eligible"),e.optInt("levelRequirement"),e.optString("memoryRequirement"),e.optBoolean("levelSatisfied"),e.optBoolean("memorySatisfied"),e.optString("unlockState"),o.optJSONArray("units").orEmpty().objects().map{MapUnitUiModel(it.optString("unitId"),it.optInt("ordinal"),it.optString("semanticKey"),it.optString("titleKey"),it.optString("state"),it.optLong("progress"),it.optLong("requiredProgress"),it.optLong("revision"))},o.optLong("revision"))}
...
private fun JSONArray?.stringValues():List<String>=if(this==null)emptyList()else List(length()){i->optString(i)}
private fun JSONArray.objects():List<JSONObject>=List(length()){i->getJSONObject(i)}
