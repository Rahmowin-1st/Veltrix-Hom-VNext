...
class ChatRepository(private val db: Database) {
    ...
    fun finishAssistant(accountId:String,messageId:String):MessageResponse = transition(accountId,messageId,MessageState.COMPLETED, final=true,emitMeaningfulChat=true)
    ...
    private fun transition(accountId:String,messageId:String,target:MessageState,final:Boolean=false,emitMeaningfulChat:Boolean=false):MessageResponse = db.tx{c->
        ...
        if(emitMeaningfulChat && updated.role=="ASSISTANT" && updated.state=="COMPLETED" && updated.content.trim().length>=40){
            val parent=updated.parentMessageId?.let{id->c.prepareStatement("SELECT content FROM conversation_message WHERE id=?::uuid AND account_id=?::uuid AND role='USER'").use{ps->ps.setString(1,id);ps.setString(2,accountId);ps.executeQuery().use{rs->if(rs.next())rs.getString(1)else null}}}
            if((parent?.trim()?.length?:0)>=10){val project=c.prepareStatement("SELECT project_id FROM conversation WHERE id=?::uuid AND account_id=?::uuid").use{ps->ps.setString(1,updated.conversationId);ps.setString(2,accountId);ps.executeQuery().use{rs->rs.next();rs.getObject(1)?.toString()}};insertActivity(c,accountId,"MEANINGFUL_CHAT_SESSION",project,updated.id,"meaningful-chat:${updated.id}",true)}
        }
        updated
    }
...
class NoteRepository(private val db:Database){
    fun create(accountId:String,req:CreateNoteRequest):NoteResponse=db.tx{c->validateLinks(c,accountId,req.projectId,req.sourceId,req.conversationId);val title=req.title.trim();if(title.isEmpty()||title.length>240)throw validation("Invalid note title");if(req.body.length>1_000_000)throw validation("Note too large");val note=c.prepareStatement("INSERT INTO note(account_id,project_id,source_id,conversation_id,title,body) VALUES (?::uuid,?::uuid,?::uuid,?::uuid,?,?) RETURNING *").use{ps->ps.setString(1,accountId);ps.setString(2,req.projectId);ps.setString(3,req.sourceId);ps.setString(4,req.conversationId);ps.setString(5,title);ps.setString(6,req.body);ps.executeQuery().use{rs->rs.next();map(rs)}};insertActivity(c,accountId,"NOTE_CREATED",note.projectId,note.id,"note-created:${note.id}",req.body.trim().length>=40);note}
