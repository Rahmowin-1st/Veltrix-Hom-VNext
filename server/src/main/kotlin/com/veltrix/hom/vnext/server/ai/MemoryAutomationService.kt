...
class MemoryAutomationService(private val db: Database, workerEnabled: Boolean = true) : Closeable {
    ...
    private fun processCandidate(c:Connection,candidateId:String,accountId:String,scope:MemoryScope,scopeId:String?,category:MemoryCategory,statement:String,canonical:String,origin:MemoryOrigin,confidence:Double,evidenceType:String,evidenceIds:List<String>) {
        ...
        val memoryId = if(existing!=null){
            ...
            c.prepareStatement("INSERT INTO memory_item(account_id,scope,scope_id,category,canonical_statement,origin,confidence,status,last_confirmed_at) VALUES (?::uuid,?,?::uuid,?,?,?,?,?,now()) RETURNING id::text").use{ps->ps.setString(1,accountId);ps.setString(2,scope.name);ps.setString(3,scopeId);ps.setString(4,category.name);ps.setString(5,statement.take(2_000));ps.setString(6,origin.name);ps.setDouble(7,confidence);ps.setString(8,status);ps.executeQuery().use{rs->rs.next();rs.getString(1)}}
        }
