...
class ProjectRepository(private val db: Database) {
    fun create(accountId: String, req: CreateProjectRequest): ProjectResponse {
        ...
        return db.tx { c ->
            c.prepareStatement("INSERT INTO project(account_id,title,purpose,template_type,priority) VALUES (?::uuid,?,?,?,?) RETURNING *").use { ps ->
    ...
    fun transitionGoal(accountId:String,projectId:String,goalId:String,target:String,expectedRevision:Long):GoalResponse = db.tx { c ->
        ...
        c.prepareStatement("SELECT * FROM goal WHERE id=?::uuid").use { ps -> ps.setString(1,goalId); ps.executeQuery().use {rs->rs.next();mapGoal(rs)} }
    }
...
class SourceRepository(private val db: Database) {
    ...
    fun ingestText(accountId:String, sourceId:String, text:String):SourceResponse{
        ...
        return db.tx{c->
            ...
            c.prepareStatement("UPDATE source SET state='PROCESSING',processing_progress=75,updated_at=now(),revision=revision+1 WHERE id=?::uuid RETURNING *").use{ps->ps.setString(1,sourceId);ps.executeQuery().use{rs->rs.next();mapSource(rs)}}
        }
    }
    ...
    fun ingestExtractedText(accountId:String, sourceId:String, text:String):SourceResponse {
        ...
        return db.tx{c->
            ...
            c.prepareStatement("UPDATE source SET state='PROCESSING',processing_progress=75,updated_at=now(),revision=revision+1 WHERE id=?::uuid AND account_id=?::uuid RETURNING *").use{ps->ps.setString(1,sourceId);ps.setString(2,accountId);ps.executeQuery().use{rs->rs.next();mapSource(rs)}}
        }
    }
