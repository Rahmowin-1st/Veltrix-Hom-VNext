...
private const val SERVICE_VERSION = "0.1.0-part1"
...
fun Application.veltrixModule(config: ServerConfig) {
    ...
    val home = HomeAggregatorRepository(db, projects, memory)
    ...
    val personal = PersonalAggregatorRepository(db,memory,timeline)
    ...
    routing {
        ...
        route("/$API_VERSION") {
            ...
            get("/store") { call.principal(auth,limiter); call.respond(HttpStatusCode.NotImplemented,ErrorEnvelope(ErrorBody("PART2_NOT_AVAILABLE","NOT_FOUND","Store economy is intentionally not implemented in Part 1",false,call.callId))) }
        }
    }
}
