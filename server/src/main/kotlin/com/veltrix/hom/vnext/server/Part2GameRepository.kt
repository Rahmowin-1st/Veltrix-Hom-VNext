...
class Part2GameRepository(private val db:Database, private val memory:MemoryRepository) {
    ...
    fun store(accountId:String):StoreCatalogResponse = db.tx { c ->
        ...
        val items=c.prepareStatement("""SELECT s.item_id,i.item_type,s.price_coins,s.requirements::text,i.metadata::text,(o.item_id IS NOT NULL),COALESCE((s.requirements->>'minLevel')::int,1),COALESCE(a.permanent_name,i.metadata->>'permanentName',initcap(replace(replace(s.item_id,'avatar-',''),'-',' '))) FROM store_item s JOIN inventory_catalog i ON i.item_id=s.item_id LEFT JOIN avatar_catalog a ON a.avatar_id=s.item_id LEFT JOIN inventory_ownership o ON o.account_id=?::uuid AND o.item_id=s.item_id WHERE s.catalog_version=? AND s.active=true AND i.active=true ORDER BY s.price_coins,s.item_id""").use{ps->ps.setString(1,accountId);ps.setString(2,catalog);ps.executeQuery().use{rs->buildList{while(rs.next()){val owned=rs.getBoolean(6);val min=rs.getInt(7);val available=!owned&&level>=min;val state=if(owned)"OWNED" else if(available)"AVAILABLE" else "LOCKED";val availability=StoreAvailabilityResponse(state,if(!owned&&!available)"LEVEL_REQUIRED" else "NONE",if(!owned&&!available)min else null);add(StoreItemResponse(rs.getString(1),rs.getString(2),catalog,rs.getLong(3),owned,available,rs.getString(4),rs.getString(5),rs.getString(8),availability))}}}}
    ...
    private fun effectiveLevel(c:Connection,a:String):Int=c.prepareStatement("SELECT LEAST(level,part3_max_level_for_days(qualified_active_days)) FROM progression_profile WHERE account_id=?::uuid").use{ps->ps.setString(1,a);ps.executeQuery().use{rs->rs.next();rs.getInt(1)}}
