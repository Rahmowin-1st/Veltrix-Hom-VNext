...
gradle --no-daemon -PVELTRIX_API_BASE_URL=http://127.0.0.1:8080 -Pandroid.testInstrumentationRunnerArguments.class=com.veltrix.hom.vnext.RootResetRuntimeInstrumentedTest :android:app:connectedDebugAndroidTest > "$EVIDENCE_DIR/root-reset-instrumentation.txt" 2>&1 || { cat "$EVIDENCE_DIR/root-reset-instrumentation.txt"; exit 1; }
...
tests=failures=errors=skipped=0; matched=[]
for path in files:
    try: root=ET.parse(path).getroot()
    except ET.ParseError: continue
    cases=[c for c in root.iter('testcase') if 'RootResetRuntimeInstrumentedTest' in c.attrib.get('classname','')]
    if not cases: continue
    matched.append(path); tests+=len(cases)
...
EOF
cat > "$EVIDENCE_DIR/stage50-gate.txt" <<'EOF'
PERSONAL_IDENTITY_FIRST=PASS
PERSISTENT_CHARACTER_IDENTITY=PASS
PERSONAL_GROWTH_REAL_DATA=PASS
MAP_BACKEND_GATING_PRESERVED=PASS
